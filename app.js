  ...
  return `
    <div class="recipe" data-mealid="${meal.idMeal}">
      ...
      <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
        <a href="${meal.strSource || meal.strYoutube || '#'}" target="_blank" class="link">Full Recipe →</a>
...
function getIngredients(meal) {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (item) list.push(measure ? `${item} — ${measure}` : item);
  }
  return list;
}
...
async function downloadRecipePdf(meal) {
  ...
  doc.text("🍳 My Recipe Book", margin, 70);
  ...
  if (meal.strYoutube) {
    ...
    doc.textWithLink("🎥 Watch tutorial", margin, y, { url: meal.strYoutube });
  }

  // Footer
  doc.setFontSize(10);
  doc.setTextColor(100);
  doc.text("Generated by Glen Recipe Book ", width - 220, height - 20);
